    ...
    return (
      <div className="flex items-center h-full px-2 bg-card">
        <select
          ...
          value={selVal}
          onChange={onSel}
          className="w-full h-7 px-1 text-sm border border-border rounded bg-card focus:outline-none focus:border-info"
        >
          <option value="">--</option>
          {opts.map((o) => (
  ...
  return (
    <div className="flex items-center h-full gap-1.5 px-2 bg-card w-[200px]">
      ...
      <button
        type="button"
        onMouseDown={toggle}
        className="shrink-0 text-[11px] text-info hover:opacity-80 hover:underline whitespace-nowrap"
      >
