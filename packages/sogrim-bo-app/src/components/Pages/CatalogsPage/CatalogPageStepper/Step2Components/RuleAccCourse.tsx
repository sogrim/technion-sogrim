...
interface RuleAccCourseProps {
  bankName: string;
  bankCourses: number;
}
const RuleAccCourseComp: React.FC<RuleAccCourseProps> = ({
  bankName,
  bankCourses,
}) => {
  const {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", mt: 1, mb: 4 }}>
      ...
      >
        <Typography>
          בבנק זה הינו בחירה - צבירת קורסים. כלומר, על הסטודנט להשלים סך קורסים
          מתוך קבוצת הקורסים של בנק דרישה זה.
        </Typography>
      </Box>
      <TextField
...
export const RuleAccCourse = observer(RuleAccCourseComp);
